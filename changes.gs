...
const err = ws_copy.getRange(1, 5); // ячейка E1 на втором листе, содержащая в себе переменную, в которой будет храниться сообщение об ошибке (даты двух массивов будут одинаковыми)
...
function copyData() { // второй лист
  ...
  if (diff_date < 0) {
    ...
    copy_date_A = ws_copy.getRange(1, 1).setValue(date_pars); // меняем дату в ячейке A1 на сегодняшнюю
  ...
  if (diff_date > 0) {
    ...
    copy_date_B = ws_copy.getRange(1, 2).setValue(date_pars); // меняем дату в ячейке B1 на сегодняшнюю
    clear_error = err.setValue(""); // очищаем ячейку с ошибкой (ячейка E1 на втором листе), так как ошибки нет
  }
  if (diff_date == 0) {
...
function compare() { // третий лист
  ...
  if (val_1 != "") {
    ...
    ws_compare.getRange(3, 1).setValue('\n\n\n\nИзменений нет.');
  }
  if (val_2 != "") {
    ...
    ws_compare.getRange(3, 2).setValue('\n\n\n\nИзменений нет.');
  }
...
function message() { // формируем запись для отправки в Telegram
  ...
  mess = "Ошибка. Необходимо проверить даты на втором листе и очистить ячейку E1"; // формирование сообщения об ошибке
  //mess = `Изменения за <u>${date_1_parse}</u>\n\n${data_T1}Изменения за <u>${date_2_parse}</u>\n\n${data_T2}`;
  if (date_1 - date_2 > 0) { // определяем разницу между датами, чтобы в первом сообщении выводились данные по более ранней дате, а во втором сообщении данные за следующий день
  ...
  if (date_1 - date_2 == 0 || err == "Ошибка") {
    sendText(clientIdChat, mess);
  }
}
